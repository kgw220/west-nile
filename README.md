...
The first task is to gather the data. Mosquito trap data is from here, which I retrieve via the
City of Chicago's API 
(ttps://data.cityofchicago.org/Health-Human-Services/West-Nile-Virus-WNV-Mosquito-Test-Results/jqe8-8r6s/about_data).
Originally, I had wanted to gather as much updated information 
as possible, but I had issues with gathering related data regarding spray applications from GIS, and weather data 
from NOAA. So, the data I have covers the period from 2007 - 2014; I hope to come back to this project
at a future date to try and fix these issues, so the insights are up to date. TODO comments were annotated in this first step.
